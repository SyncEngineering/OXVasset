# Session 02 — Database Connection & Sequence Utility

## Context
Stack: Node.js, Express, MySQL (mysql2/promise), React, plain CSS.
Session 1 has been completed. The project scaffold exists.
This session builds two things only: the MySQL schema runner and the auto-sequence utility.
Do not build any feature routes or React pages.

---

## Task 1 — MySQL Schema

Create the file: backend/src/config/schema.sql

Paste this schema exactly as written. Do not modify column names, types, or constraints.
...
CREATE INDEX IF NOT EXISTS idx_asset_master_division  ON tbl_asset_master (division_id);
CREATE INDEX IF NOT EXISTS idx_asset_master_category  ON tbl_asset_master (category_id);
CREATE INDEX IF NOT EXISTS idx_asset_master_status    ON tbl_asset_master (asset_status);
CREATE INDEX IF NOT EXISTS idx_asset_master_location  ON tbl_asset_master (location_id);
CREATE INDEX IF NOT EXISTS idx_depreciation_asset     ON tbl_depreciation_entry (asset_id);
CREATE INDEX IF NOT EXISTS idx_depreciation_period    ON tbl_depreciation_entry (period_from, period_to);
CREATE INDEX IF NOT EXISTS idx_transfer_asset         ON tbl_asset_transfer (asset_id);
CREATE INDEX IF NOT EXISTS idx_expiry_doc_date        ON tbl_expiry_document_entry (expiry_date);
CREATE INDEX IF NOT EXISTS idx_company_license_expiry ON tbl_company_license_documents (expiry_date);
```

---

## Task 2 — Schema Runner Script

Create: backend/src/config/runSchema.js

- Read schema.sql from same directory using fs.readFileSync
- Split by semicolon to get individual statements
- Filter out empty strings
- Get a connection from the pool in db.js
- Execute each statement in sequence using a for loop with await
- Log each table name as it runs
- Release connection when done
- On error log and exit process with code 1
- Export an async function runSchema
- At bottom: call runSchema() directly so node src/config/runSchema.js runs it

---

## Task 3 — Sequence Utility

Create: backend/src/utils/sequenceGenerator.js

Write and export one async function: generateDocNumber(pool, docType)

Logic:
1. Get a connection from pool
2. BEGIN TRANSACTION
3. SELECT last_sequence, prefix FROM tbl_asset_doc_sequence WHERE doc_type = ? FOR UPDATE
4. If no row found throw new Error with message "Unknown doc type: " + docType
5. Increment last_sequence by 1
6. UPDATE tbl_asset_doc_sequence SET last_sequence = newSeq WHERE doc_type = ?
7. COMMIT
8. Release connection
9. Return string: prefix + "/" + String(newSeq).padStart(5, "0")
   Example output: DEP/00001, TRF/00003

On any error: ROLLBACK, release connection, rethrow error.

---

## Output rules
- Generate all three files completely
- Do not modify the schema SQL in any way
- Do not add any Express routes or React files
- Comment the utility function steps clearly
